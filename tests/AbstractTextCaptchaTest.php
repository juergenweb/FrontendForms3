 ...
 *
 * getBackgroundType()/getBackgroundColor()/getNumberOfColors()/getTextColor()/
...
final class AbstractTextCaptchaTest extends TestCase
{
    ...
    {
        $captcha = new ConcreteTextCaptcha();
        $captcha->exposeSetNumberOfColors(4);

        $this->assertSame(4, $captcha->exposeGetNumberOfColors());
    }
     ...
     */
    public function testGetNumberOfColorsCastsConfigToInt(): void
    {
        $captcha = new ConcreteTextCaptcha();
        $this->setConfig($captcha, ['input_bgnumberOfColors' => '3']);

        $this->assertSame(3, $captcha->exposeGetNumberOfColors());
    }
