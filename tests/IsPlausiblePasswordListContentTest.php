...
/**
 * Unit tests for FrontendForms::isPlausiblePasswordListContent(), the
 * defense-in-depth sanity check applied to content downloaded from GitHub
 * before it gets saved to passwords.txt.
 *
...
{
    private function call(string $content): bool
    {
        $ref = new ReflectionClass(FrontendForms::class);
        $module = $ref->newInstanceWithoutConstructor();

        $method = new ReflectionMethod(FrontendForms::class, 'isPlausiblePasswordListContent');
        $method->setAccessible(true);
        return $method->invoke($module, $content);
    }
