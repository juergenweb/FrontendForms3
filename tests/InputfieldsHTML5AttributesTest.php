...
final class InputfieldsHTML5AttributesTest extends TestCase
{
     ...
     * text, aria-describedby references BOTH of their ids (space-
     * separated), not just the last one set. Previously, setting
     * aria-describedby a second time (for notes) silently overwrote the
     * first value (for the description) instead of combining them, so a
     * screen reader user was never told about the description at all.
     */
    public function testAriaDescribedByCombinesDescriptionAndNotesIds(): void
    {
        ...
        $this->assertStringContainsString($field->getID() . '-desc', $describedBy);
        $this->assertStringContainsString($field->getID() . '-notes', $describedBy);
    }
